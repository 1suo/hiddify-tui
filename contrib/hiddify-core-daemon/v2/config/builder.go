...
func setOutbounds(options *option.Options, input *option.Options, opt *HiddifyOptions, staticIPs *map[string][]string) error {
	...
	if len(tags) == 0 {
		return fmt.Errorf("profile contains no selectable proxy outbounds")
	}
	...
	options.Outbounds = append(
		...
		[]option.Outbound{
			...
			},
			{
				...
				Options: &option.DirectOutboundOptions{
					DialerOptions: option.DialerOptions{
						// sing-box rejects a detoured direct outbound when all of
						// its dialer options are defaults. Keep this dedicated
						// outbound explicit without changing its routing behavior.
